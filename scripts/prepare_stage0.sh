...
if [[ $# -ne 3 ]]; then
  echo "Usage: $0 /abs/prior_ml.csv /abs/reported_positive.tsv /abs/weak_reference.csv" >&2
  exit 2
fi
...
"${PYTHON_BIN}" -m geroprotector.cli --root "${ROOT_DIR}" splits build
"${PYTHON_BIN}" -m geroprotector.cli --root "${ROOT_DIR}" splits audit
